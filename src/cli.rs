            ...
            if output_path.exists() {
                ...
                match choice.trim().to_lowercase().as_str() {
                    ...
                    "a" | "add" => {
                        ...
                        let mut new_content_reader: Box<File> = if input_path.is_dir() { // Changed to mut Box<File>
                            let temp_archive_name = format!("{}.tmp_new_content", &output_path_str);
                            let temp_archive_path = Path::new(&temp_archive_name);
                            let temp_archive_file = File::create(temp_archive_path).map_err(|e| {
                            ...
                            Box::new(File::open(temp_archive_path).map_err(|e| {
                                eprintln!("Error opening temporary archive file '{}': {}", temp_archive_path.display(), e);
                                Box::new(e) as Box<dyn std::error::Error>
                            })?)
                        } else {
                            ...
                            Box::new(File::open(temp_file_path).map_err(|e| {
                                eprintln!("Error opening temporary file '{}': {}", temp_file_path.display(), e);
                                Box::new(e) as Box<dyn std::error::Error>
                            })?)
                        };

                        // 2. Prepare existing archive reader
                        let mut existing_archive_reader = BufReader::new(File::open(output_path).map_err(|e| {
                            eprintln!("Error opening existing archive '{}': {}", output_path.display(), e);
                            Box::new(e) as Box<dyn std::error::Error>
                        })?);

                        // 3. Prepare temporary output writer
                        let temp_output_path_str = format!("{}.tmp", &output_path_str);
                        let temp_output_file = File::create(&temp_output_path_str).map_err(|e| {
                            eprintln!("Error creating temporary output file '{}': {}", temp_output_path_str, e);
                            Box::new(e) as Box<dyn std::error::Error>
                        })?;
                        ...
                        fs::rename(&temp_output_path_str, output_path).map_err(|e| {
                            eprintln!("Error replacing original archive with updated archive: {}", e);
                            Box::new(e) as Box<dyn std::error::Error>
                        })?;
